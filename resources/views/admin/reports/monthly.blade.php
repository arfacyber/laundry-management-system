<x-app-layout>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button onclick="window.print()" class="bg-gray-800 text-white px-4 py-2 rounded text-sm font-medium hover:bg-gray-700 print:hidden">
            Cetak Laporan
        </button>
    </div>
    ...
    <div class="bg-white p-4 rounded-lg border border-gray-200 mb-6 print:hidden">
        <form action="{{ route('admin.reports.monthly') }}" method="GET" class="flex gap-4 items-end">
            <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div class="bg-white p-6 rounded-lg border border-gray-200">
            ...
            <h3 class="text-3xl font-bold text-gray-900 mt-1">{{ $totalTransaksi }} Pesanan</h3>
        </div>
        <div class="bg-white p-6 rounded-lg border border-gray-200">
            <p class="text-sm text-gray-500">Total Pendapatan Masuk (Kas)</p>
            <h3 class="text-3xl font-bold text-green-600 mt-1">Rp {{ number_format($totalPendapatan, 0, ',', '.') }}</h3>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50 text-gray-500 uppercase text-xs">
                <tr>
                    <th class="px-6 py-3 text-left">Kode TRX</th>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                ...
                <tr>
                    <td class="px-6 py-4 font-bold">{{ $trx->transaction_code }}</td>
                    <td class="px-6 py-4">{{ $trx->customer->nama }}</td>
                    <td class="px-6 py-4">{{ $trx->tanggal_masuk->format('d/m/Y') }}</td>
                    <td class="px-6 py-4">{{ $trx->status_laundry }}</td>
                    <td class="px-6 py-4 text-right font-medium">Rp {{ number_format($trx->total, 0, ',', '.') }}</td>
                </tr>
                @empty
                <tr>
