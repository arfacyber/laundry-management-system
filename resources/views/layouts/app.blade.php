...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <!-- Tambahkan x-data untuk mendeteksi status buka/tutup menu di HP -->
    <body class="font-sans antialiased bg-slate-50 text-slate-900 overflow-hidden" x-data="{ sidebarOpen: false }">
        <div class="flex h-screen w-full">
            
            <!-- Latar belakang gelap (Overlay) saat menu terbuka di HP -->
            ...
            <!-- Sidebar Navigation (Otomatis disembunyikan di layar kecil) -->
            <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-30 w-64 bg-white shadow-2xl flex flex-col transition-transform duration-300 lg:static lg:translate-x-0">
                <!-- Logo -->
                <div class="h-16 lg:h-20 flex items-center px-8 border-b border-slate-100 justify-between">
                    <div class="flex items-center">
                        ...
                        <span class="text-xl lg:text-2xl font-extrabold text-slate-800 tracking-tight">LaundryPro</span>
                    </div>
                    <!-- Tombol Tutup (Hanya tampil di HP) -->
                    <button @click="sidebarOpen = false" class="lg:hidden text-slate-400 hover:text-red-500">
            ...
            <!-- Main Content Area -->
            <main class="flex-1 flex flex-col h-screen overflow-x-hidden overflow-y-auto bg-slate-50 w-full">
                <!-- User Top Bar dengan Tombol Hamburger untuk HP -->
                <div class="h-16 bg-white border-b border-slate-100 flex items-center justify-between lg:justify-end px-4 lg:px-8 shadow-sm">
                    <!-- Tombol Buka Menu (Tampil di Layar Kecil) -->
                    <button @click="sidebarOpen = true" class="lg:hidden p-2 text-slate-500 hover:text-blue-600 hover:bg-slate-50 rounded-lg focus:outline-none">
